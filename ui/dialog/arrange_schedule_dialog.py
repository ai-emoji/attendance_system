    ...
    def __init__(self, parent=None) -> None:
        ...
        def _list_shift_codes_from_db() -> list[str]:
            ...
            try:
                ...
                logging.getLogger(__name__).exception("Không thể tải danh sách ca từ work_shifts")
                return []
            finally:
        ...
        def _init_selected_tree_days() -> None:
            ...
            for day_name in DAY_NAMES:
                ...
                node.setData(0, Qt.ItemDataRole.UserRole, {"kind": "day", "text": day_name})
        ...
        def _get_day_node(day_name: str) -> QTreeWidgetItem | None:
            ...
            for i in range(self.tree_selected.topLevelItemCount()):
                ...
                if str(meta.get("kind") or "") == "day" and _norm_day(str(meta.get("text") or "")) == _norm_day(day_name):
                    return node
            return None

        def _find_child_shift(day_node: QTreeWidgetItem, shift_code: str) -> QTreeWidgetItem | None:
            ...
            for i in range(day_node.childCount()):
                ...
                if str(meta.get("kind") or "") == "shift" and _norm_code(str(meta.get("text") or "")) == _norm_code(shift_code):
                    return ch
            return None
        ...
        def _on_apply_selected() -> None:
            ...
            if not shift_codes:
                MessageDialog.info(self, "Thông báo", "Vui lòng tích chọn ít nhất 1 ca.")
                return
            if not day_names:
                MessageDialog.info(self, "Thông báo", "Vui lòng tích chọn ít nhất 1 ngày.")
                return
        ...
        def _seed_default_work_shifts(codes_to_seed: list[str]) -> bool:
            ...
            templates: dict[str, dict[str, object]] = {
                "hc": {"shift_code": "HC", "time_in": "08:00:00", "time_out": "17:00:00", "work_count": 1.0},
                "sa": {"shift_code": "SA", "time_in": "08:00:00", "time_out": "12:00:00", "work_count": 0.5},
                "ch": {"shift_code": "CH", "time_in": "13:00:00", "time_out": "17:00:00", "work_count": 0.5},
                "ca1": {"shift_code": "Ca1", "time_in": "06:00:00", "time_out": "14:00:00", "work_count": 1.0},
                "ca2": {"shift_code": "Ca2", "time_in": "14:00:00", "time_out": "22:00:00", "work_count": 1.0},
            }
            ...
            try:
                ...
                logging.getLogger(__name__).exception("Không thể seed work_shifts mặc định")
                return False
            finally:
                if cursor is not None:
                    try:
        ...
        def _on_apply() -> None:
            ...
            if not mapping:
                MessageDialog.info(self, "Thông báo", "Chưa có dữ liệu trong 'Danh sách ca chọn'.")
                return
            ...
            unique_codes = sorted(set([_norm_code(c) for c in all_codes if _norm_code(c)]))
            ...
            for r in range(table.rowCount()):
                ...
                for idx, col in enumerate(shift_cols):
                    ...
                    sid, code_text = shift_pairs[idx] if idx < len(shift_pairs) else (None, "")
        ...
        self.tree_selected.itemClicked.connect(lambda item, _col: _tree_toggle_item(item))
        ...
        def _load_selected_from_parent_table() -> None:
            ...
            for r in range(table.rowCount()):
                ...
                for c in (2, 3, 4, 5, 6):
                    ...
                    child = QTreeWidgetItem([""])
                    child.setData(0, Qt.ItemDataRole.UserRole, {"kind": "shift", "text": code})
                    day_node.addChild(child)
                    _tree_set_checked(child, True)
                    any_added = True
